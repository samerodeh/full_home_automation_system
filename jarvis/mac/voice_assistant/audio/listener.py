...
class Listener:
    ...
    # -- stream lifecycle ----------------------------------------------------
    def _input_device(self, sd):
        """Index of the configured mic (substring match on name), or None for default."""
        want = (config.INPUT_DEVICE or "").strip().lower()
        if want:
    ...
    def start(self):
        if self._frame_source is None:
            ...
            self._stream = sd.InputStream(
                ...
                blocksize=self.frame_len, device=self._input_device(sd),
            )
            self._stream.start()
        self._calibrate()
        return self
